...
def collate_batch(data: list[torch.LongTensor]) -> Batch:
    data = torch.stack([b for b in data])
    ...
    def get_grouped_dataset(f: str, seq_len: int) -> GroupedDataset:
        if f.endswith("npy"):
            ...
            if len(matching_files) == 0:
                assert len(matching_files) > 0, f"Unable to find {f} in {files}"
...
def get_num_samples_in_batch(batch: Batch):
    return batch["input_ids"].shape[0]


def get_num_tokens_in_batch(batch: Batch):
    return batch["input_ids"].shape[0] * (batch["input_ids"].shape[1] + 1)


def split_batch(batch: Batch, microbatch_size):
